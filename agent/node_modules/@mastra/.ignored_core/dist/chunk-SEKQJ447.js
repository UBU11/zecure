import { createTool } from './chunk-ZVWVQ6MG.js';
...
function expandTilde(p) {
  ...
  if (p.startsWith("~/") || p.startsWith("~\\")) {
    return nodePath.join(os3.homedir(), p.slice(2));
  }
  return p;
}
...
var MARKER_DIR = nodePath.join(os3.tmpdir(), ".mastra-mounts");
...
var LocalSandbox = class extends MastraSandbox {
  ...
  constructor(options = {}) {
    ...
    this.workingDirectory = expandTilde(options.workingDirectory ?? nodePath.join(process.cwd(), ".sandbox"));
  ...
  async mount(filesystem, mountPath) {
    ...
    try {
      const entries = await fs2.readdir(hostPath);
      if (entries.length > 0) {
    ...
    try {
      await fs2.mkdir(nodePath.dirname(hostPath), { recursive: true });
      await fs2.symlink(localConfig.basePath, hostPath);
  ...
  async unmount(mountPath) {
    ...
    let isSymlink = false;
    try {
      const stats = await fs2.lstat(hostPath);
    ...
    const markerPath = nodePath.join(MARKER_DIR, filename);
    try {
      await fs2.unlink(markerPath);
    } catch {
    }
    if (isSymlink) {
      try {
        await fs2.unlink(hostPath);
  ...
  async writeMarkerFile(mountPath, hostPath) {
    ...
    const markerFilePath = nodePath.join(MARKER_DIR, filename);
    try {
      await fs2.mkdir(MARKER_DIR, { recursive: true });
      await fs2.writeFile(markerFilePath, markerContent, "utf-8");
    } catch {
  ...
  async checkExistingMount(hostPath, newConfig) {
    try {
      ...
      if (stats.isSymbolicLink() && newConfig.type === "local") {
        const linkTarget = await fs2.readlink(hostPath).catch(() => null);
  ...
  async checkMarkerFile(hostPath, newConfig) {
    ...
    const markerPath = nodePath.join(MARKER_DIR, filename);
    try {
      const content = await fs2.readFile(markerPath, "utf-8");
...
//# sourceMappingURL=chunk-SEKQJ447.js.map
//# sourceMappingURL=chunk-SEKQJ447.js.map