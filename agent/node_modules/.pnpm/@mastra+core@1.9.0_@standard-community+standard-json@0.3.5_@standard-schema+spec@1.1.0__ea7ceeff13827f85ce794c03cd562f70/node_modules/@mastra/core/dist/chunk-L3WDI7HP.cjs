...
var chunkYIJZBU54_cjs = require('./chunk-YIJZBU54.cjs');
...
var StepExecutor = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  async execute(params) {
    ...
    const { inputData, validationError } = await chunkYIJZBU54_cjs.validateStepInput({
    ...
    try {
      ...
      const stepOutput = await step.execute(
        chunkYIJZBU54_cjs.createDeprecationProxy(
          {
            ...
            getStepResult: chunkYIJZBU54_cjs.getStepResult.bind(this, stepResults),
            suspend: async (suspendPayload, suspendOptions) => {
              const { suspendData, validationError: validationError2 } = await chunkYIJZBU54_cjs.validateStepSuspendData({
            ...
            // TODO
            ...chunk4UOWNFAL_cjs.createObservabilityContext()
          },
          {
            paramName: "runCount",
            deprecationMessage: chunkYIJZBU54_cjs.runCountDeprecationMessage,
            logger: this.logger
          }
        )
      );
      ...
      return {
        ...
        tripwire: error instanceof chunkYIJZBU54_cjs.TripWire ? {
...
function createTripWireFromChunk(chunk) {
  const { payload } = chunk;
  return new chunkYIJZBU54_cjs.TripWire(
  ...
  if (resumeData !== void 0 && forEachIndex === void 0 && currentResult?.output?.length > 0) {
    ...
    if (suspendedIndices.length > 0) {
      ...
      for (const suspIdx of indicesToResume) {
        updatedOutput[suspIdx] = chunkZBESCKPX_cjs.createPendingMarker();
      }
  ...
  async processWorkflowStepRun({
    ...
    if (step.step instanceof EventedWorkflow || step.step.component === "WORKFLOW") {
      if (resumeSteps?.length === 1 && resumeSteps[0] === step.step.id) {
        ...
        const timeTravelParams = chunkYIJZBU54_cjs.createTimeTravelExecutionParams({
    ...
    const { resumeData: timeTravelResumeData, validationError: timeTravelResumeValidationError } = await chunkYIJZBU54_cjs.validateStepResumeData({
...
function cloneWorkflow(workflow, opts) {
  const wf = new chunkYIJZBU54_cjs.Workflow({
    id: opts.id,
...
function isAgent(input) {
  return input instanceof chunkYIJZBU54_cjs.Agent;
}
function isToolStep(input) {
  return input instanceof chunkD6HO5QAM_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof chunkYIJZBU54_cjs.Agent) && !(input instanceof chunkD6HO5QAM_cjs.Tool);
}
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof chunkYIJZBU54_cjs.Agent) && !(obj instanceof chunkD6HO5QAM_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
...
function createStepFromAgent(params, agentOrToolOptions) {
  ...
  return {
    ...
    execute: async ({
      ...
      const observabilityContext = chunk4UOWNFAL_cjs.resolveObservabilityContext(obsFields);
      const logger = mastra?.getLogger();
      const toolData = {
        name: params.name,
        args: inputData
      ...
      const isV2Model = chunkYIJZBU54_cjs.isSupportedLanguageModel(modelInfo);
...
function createStepFromProcessor(processor) {
  ...
  return {
    ...
    inputSchema: chunkYIJZBU54_cjs.ProcessorStepSchema,
    outputSchema: chunkYIJZBU54_cjs.ProcessorStepOutputSchema,
    execute: async ({ inputData, requestContext, ...obsFields }) => {
      ...
      const abort = (reason, options) => {
        throw new chunkYIJZBU54_cjs.TripWire(reason || `Tripwire triggered by ${processor.id}`, options, processor.id);
      };
      ...
      const executePhaseWithSpan = async (fn) => {
        try {
          ...
          if (error instanceof chunkYIJZBU54_cjs.TripWire) {
        ...
        switch (phase) {
          case "input": {
            if (processor.processInput) {
              ...
              if (result instanceof chunk3ZF7IC6Q_cjs.MessageList) {
                ...
                chunkYIJZBU54_cjs.ProcessorRunner.applyMessagesToMessageList(
                  result,
          ...
          case "inputStep": {
            if (processor.processInputStep) {
              ...
              const validatedResult = await chunkYIJZBU54_cjs.ProcessorRunner.validateAndFormatProcessInputStepResult(result, {
                messageList: passThrough.messageList,
                processor,
                stepNumber: stepNumber ?? 0
              });
...
var EventedWorkflow = class extends chunkYIJZBU54_cjs.Workflow {
  constructor(params) {
    super(params);
...
var EventedRun = class extends chunkYIJZBU54_cjs.Run {
  constructor(params) {
    super(params);
  ...
  stream({
    ...
    this.streamOutput = new chunkYIJZBU54_cjs.WorkflowRunOutput({
      runId: this.runId,
      workflowId: this.workflowId,
      stream
    });
...
var EventedExecutionEngine = class extends chunkYIJZBU54_cjs.ExecutionEngine {
  eventProcessor;
  constructor({
    mastra,
    eventProcessor,
  ...
  async execute(params) {
    ...
    const finishCb = async (event, ack) => {
      ...
      if (["workflow.end", "workflow.fail", "workflow.suspend"].includes(event.type)) {
        ...
        if (event.type === "workflow.fail" && event.data.stepResults) {
          event.data.stepResults = chunkYIJZBU54_cjs.hydrateSerializedStepErrors(event.data.stepResults);
        }
    ...
    for (const [stepId, stepResult] of Object.entries(stepResultsWithoutTopLevelState)) {
      cleanStepResults[stepId] = chunkYIJZBU54_cjs.cleanStepResult(stepResult);
    }
    let callbackArg;
...
//# sourceMappingURL=chunk-L3WDI7HP.cjs.map
//# sourceMappingURL=chunk-L3WDI7HP.cjs.map