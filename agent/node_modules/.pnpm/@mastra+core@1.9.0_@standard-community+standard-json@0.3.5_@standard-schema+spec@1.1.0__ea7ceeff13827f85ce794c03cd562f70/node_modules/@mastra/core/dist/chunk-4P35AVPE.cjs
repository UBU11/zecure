...
var LocalFilesystem = class extends MastraFilesystem {
  ...
  setAllowedPaths(pathsOrUpdater) {
    ...
    this._allowedPaths = newPaths.map((p) => nodePath__namespace.resolve(expandTilde(p)));
  }
  constructor(options) {
    ...
    this._allowedPaths = (options.allowedPaths ?? []).map((p) => nodePath__namespace.resolve(expandTilde(p)));
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.type === "directory") {
        results.push(...await walkAll(readdir4, fullPath, depth + 1, maxDepth));
      }
    }
    return results;
  } catch {
...
async function resolvePathPattern(pattern, readdir4, options) {
  ...
  const allEntries = await walkAll(readdir4, walkRoot, 0, maxDepth);
...
function whichSync(binary) {
  try {
    ...
    child_process.execFileSync(cmd, [binary], { stdio: "ignore" });
    return true;
  } catch {
    return false;
  }
...
function resolveRequireFromPaths(root, moduleId, searchPaths) {
  ...
  for (const searchPath of searchPaths ?? []) {
    try {
      const req = module$1.createRequire(url.pathToFileURL(nodePath.join(searchPath, "package.json")));
      return { require: req, resolved: req.resolve(moduleId) };
    } catch {
    }
  }
...
function resolveNodeBin(root, binary, searchPaths) {
  const local = nodePath.join(root, "node_modules", ".bin", binary);
  const cwd = nodePath.join(process.cwd(), "node_modules", ".bin", binary);
  if (fs.existsSync(local)) return local;
  if (fs.existsSync(cwd)) return cwd;
  for (const dir of searchPaths ?? []) {
    const p = nodePath.join(dir, "node_modules", ".bin", binary);
  ...
  return {
    typescript: {
      ...
      command: (root) => {
        ...
        if (!resolveRequireFromPaths(root, "typescript/lib/tsserver.js", searchPaths)) return void 0;
...
var BUILTIN_SERVERS = buildServerDefs();
function getServersForFile(filePath, disabledServers, defs) {
...
Process timed out after ${options.timeout}ms`;
  ...
  async sendStdin(data) {
    ...
    return new Promise((resolve4, reject) => {
      this.subprocess.stdin.write(data, (err) => err ? reject(err) : resolve4());
    });
  }
};
...
// src/workspace/sandbox/mounts/types.ts
...
// src/workspace/sandbox/mount-manager.ts
...
function isSeatbeltAvailable() {
  if (os3__namespace.default.platform() !== "darwin") {
    return false;
  }
  return commandExists("sandbox-exec");
}
function isBwrapAvailable() {
  if (os3__namespace.default.platform() !== "linux") {
    return false;
  }
  return commandExists("bwrap");
}
function detectIsolation() {
  const platform2 = os3__namespace.default.platform();
  if (platform2 === "darwin") {
...
var MARKER_DIR = nodePath__namespace.join(os3__namespace.tmpdir(), ".mastra-mounts");
...
var LocalSandbox = class extends MastraSandbox {
  ...
  constructor(options = {}) {
    ...
    this.workingDirectory = expandTilde(options.workingDirectory ?? nodePath__namespace.join(process.cwd(), ".sandbox"));
  ...
  async getInfo() {
    return {
      ...
      resources: {
        memoryMB: Math.round(os3__namespace.totalmem() / 1024 / 1024),
        cpuCores: os3__namespace.cpus().length
      },
      metadata: {
        workingDirectory: this.workingDirectory,
        platform: os3__namespace.platform(),
  ...
  async mount(filesystem, mountPath) {
    ...
    try {
      const entries = await fs2__namespace.readdir(hostPath);
      if (entries.length > 0) {
      ...
      await fs2__namespace.rmdir(hostPath);
    } catch (err) {
    ...
    try {
      await fs2__namespace.mkdir(nodePath__namespace.dirname(hostPath), { recursive: true });
      await fs2__namespace.symlink(localConfig.basePath, hostPath);
  ...
  async unmount(mountPath) {
    ...
    let isSymlink = false;
    try {
      const stats = await fs2__namespace.lstat(hostPath);
    ...
    const markerPath = nodePath__namespace.join(MARKER_DIR, filename);
    try {
      await fs2__namespace.unlink(markerPath);
    } catch {
    }
    if (isSymlink) {
      try {
        await fs2__namespace.unlink(hostPath);
  ...
  async writeMarkerFile(mountPath, hostPath) {
    ...
    const markerFilePath = nodePath__namespace.join(MARKER_DIR, filename);
    try {
      await fs2__namespace.mkdir(MARKER_DIR, { recursive: true });
      await fs2__namespace.writeFile(markerFilePath, markerContent, "utf-8");
    } catch {
  ...
  async checkExistingMount(hostPath, newConfig) {
    try {
      ...
      if (stats.isSymbolicLink() && newConfig.type === "local") {
        const linkTarget = await fs2__namespace.readlink(hostPath).catch(() => null);
        const resolvedTarget = linkTarget ? nodePath__namespace.resolve(nodePath__namespace.dirname(hostPath), linkTarget) : null;
        const expectedTarget = nodePath__namespace.resolve(newConfig.basePath);
  ...
  async hasMarkerFile(hostPath) {
    ...
    const markerPath = nodePath__namespace.join(MARKER_DIR, filename);
    try {
      await fs2__namespace.access(markerPath);
      return true;
    } catch {
      return false;
    }
  ...
  async checkMarkerFile(hostPath, newConfig) {
    ...
    const markerPath = nodePath__namespace.join(MARKER_DIR, filename);
    try {
      const content = await fs2__namespace.readFile(markerPath, "utf-8");
   ...
   */
  addMountPathToIsolation(mountPath) {
  ...
  resolveHostPath(mountPath) {
    return nodePath__namespace.join(this.workingDirectory, mountPath.replace(/^\/+/, ""));
  }
  /**
  ...
  async #discoverSkills() {
    ...
    for (const rawSkillsPath of this.#resolvedPaths) {
      ...
      if (isGlobPattern(skillsPath)) {
        const resolved = await resolvePathPattern(skillsPath, readdir4, { dot: true, maxDepth: 4 });
        const dirs = /* @__PURE__ */ new Set();
...
var Workspace = class {
  ...
  async rebuildSearchIndex(paths) {
    ...
    for (const pathOrGlob of paths) {
      try {
        const resolved = await resolvePathPattern(pathOrGlob, readdir4);
        ...
        for (const entry of resolved) {
          ...
          const alreadyCovered = directoryRoots.some(
            (root) => entry.path === root || (root === "/" ? entry.path.startsWith("/") : entry.path.startsWith(`${root}/`))
          );
  ...
  getInstructions(opts) {
    ...
    if (mountEntries && mountEntries.size > 0) {
      ...
      const workspaceOnly = [];
      const workingDir = this._sandbox instanceof LocalSandbox ? this._sandbox.workingDirectory : void 0;
      for (const [mountPath, entry] of mountEntries) {
        const fsName = entry.filesystem.displayName || entry.filesystem.provider;
        const access3 = entry.filesystem.readOnly ? "read-only" : "read-write";
        const displayPath = workingDir ? nodePath__namespace.join(workingDir, mountPath.replace(/^\/+/, "")) : mountPath;
        if (entry.state === "mounted" || entry.state === "pending" || entry.state === "mounting") {
          sandboxAccessible.push(`  - ${displayPath}: ${fsName} (${access3})`);
        } else {
          workspaceOnly.push(`  - ${mountPath}: ${fsName} (${access3})`);
        }
      }
...
${workspaceOnly.join("\n")}`
  ...
  getPathContext() {
    ...
    return {
      ...
      sandbox: this._sandbox ? {
        provider: this._sandbox.provider,
        workingDirectory: this._sandbox instanceof LocalSandbox ? this._sandbox.workingDirectory : void 0
      } : void 0,
      instructions
    };
  }
...
var readFileTool = chunkD6HO5QAM_cjs.createTool({
  ...
  execute: async ({ path: path7, encoding, offset, limit, showLineNumbers }, context) => {
    ...
    if (!isTextEncoding) {
      return await applyTokenLimit(
        `${stat3.path} (${stat3.size} bytes, ${effectiveEncoding})
${fullContent}`,
        tokenLimit,
        "end"
      );
    ...
    if (typeof fullContent !== "string") {
      return await applyTokenLimit(
        `${stat3.path} (${stat3.size} bytes, base64)
