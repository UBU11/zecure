...
var chunkBN5MV2QK_cjs = require('./chunk-BN5MV2QK.cjs');
var chunkRXAQEKAJ_cjs = require('./chunk-RXAQEKAJ.cjs');
var chunkCSHG5LWU_cjs = require('./chunk-CSHG5LWU.cjs');
var chunkD4M6E4OQ_cjs = require('./chunk-D4M6E4OQ.cjs');
...
var AISDKV6LanguageModel = class {
  ...
  async doGenerate(options) {
    ...
    return {
      ...
      stream: chunkPKORY4ZZ_cjs.createStreamFromGenerateResult(result)
    };
  }
...
function parseJsonEventStream({
  ...
  return stream.pipeThrough(new TextDecoderStream()).pipeThrough(new chunkCSHG5LWU_cjs.EventSourceParserStream()).pipeThrough(
    new TransformStream({
...
var AzureOpenAIGateway = class extends chunkCSHG5LWU_cjs.MastraModelGateway {
  constructor(config) {
    super();
    this.config = config;
    this.validateConfig();
...
function createOpenAIWebSocketFetch(options) {
  ...
  let busy = false;
  function getConnection(authorization, headers) {
    const normalizedHeaders = { ...normalizeHeaders2(options?.headers), ...headers };
    ...
    connecting = new Promise((resolve, reject) => {
      const socket = new WebSocket__default.default(wsUrl, {
        headers: {
          ...normalizedHeaders,
  ...
  async function websocketFetch(input, init) {
    ...
    const headers = normalizeHeaders2(init.headers);
...
function buildConnectionKey(authorization, headers) {
...
function getOpenAITransport(providerOptions) {
...
function getStaticProvidersByGateway(name14) {
  return Object.fromEntries(Object.entries(chunkD4M6E4OQ_cjs.PROVIDER_REGISTRY).filter(([_provider, config]) => config.gateway === name14));
}
var defaultGateways = [new chunkRXAQEKAJ_cjs.NetlifyGateway(), new chunkBN5MV2QK_cjs.ModelsDevGateway(getStaticProvidersByGateway(`models.dev`))];
var ModelRouterLanguageModel = class _ModelRouterLanguageModel {
  ...
  constructor(config, customGateways) {
    ...
    const parsed = chunkBN5MV2QK_cjs.parseModelRouterId(normalizedConfig.id, gatewayPrefix);
  ...
  async _fetchSupportedUrls() {
    ...
    try {
      ...
      const model = await this.resolveLanguageModel({
        ...
        ...chunkBN5MV2QK_cjs.parseModelRouterId(this.config.routerId, gatewayPrefix)
      });
  ...
  setStreamTransport({
    ...
    const wsFetch = _ModelRouterLanguageModel.webSocketFetches.get(key);
    if (!wsFetch) {
      this.#lastStreamTransport = void 0;
      return;
    }
  ...
  async doGenerate(options) {
    ...
    const aiSDKV5Model = new chunkPKORY4ZZ_cjs.AISDKV5LanguageModel(model);
  ...
  async resolveLanguageModel({
    ...
    const websocketKey = resolvedTransport === "websocket" ? `${openaiWebSocket?.url ?? ""}:${stableHeaderKey(openaiWebSocket?.headers)}` : "";
    const key = crypto.createHash("sha256").update(
      this.gateway.id + modelId + providerId + apiKey + (this.config.url || "") + stableHeaderKey(headers) + resolvedTransport + websocketKey
    ).digest("hex");
    if (_ModelRouterLanguageModel.modelInstances.has(key)) {
      this.setStreamTransport({ resolvedTransport, key, openaiWebSocket });
      return _ModelRouterLanguageModel.modelInstances.get(key);
    }
    if (this.config.url) {
      const modelInstance2 = chunkCSHG5LWU_cjs.createOpenAICompatible({
        name: providerId,
        apiKey,
    ...
    if (resolvedTransport === "websocket" && providerId === "openai" && this.gateway.id === "models.dev") {
      ...
      if (isOpenAIBaseUrl(baseURL)) {
        ...
        _ModelRouterLanguageModel.modelInstances.set(key, modelInstance2);
        _ModelRouterLanguageModel.webSocketFetches.set(key, wsFetch);
        this.setStreamTransport({ resolvedTransport, key, openaiWebSocket });
        return modelInstance2;
      }
    }
  ...
  resolveOpenAIWebSocketModel({
    ...
    const modelInstance = chunkCSHG5LWU_cjs.createOpenAI({
      apiKey,
      baseURL,
      headers,
      fetch: wsFetch
...
async function resolveModelConfig(modelConfig, requestContext = new chunkVDKWYUGC_cjs.RequestContext(), mastra) {
  ...
  if (modelConfig instanceof ModelRouterLanguageModel || modelConfig instanceof chunkPKORY4ZZ_cjs.AISDKV5LanguageModel || modelConfig instanceof AISDKV6LanguageModel) {
    return modelConfig;
  }
  if (typeof modelConfig === "object" && "specificationVersion" in modelConfig) {
    if (modelConfig.specificationVersion === "v2") {
      return new chunkPKORY4ZZ_cjs.AISDKV5LanguageModel(modelConfig);
    }
...
var ModelRouterEmbeddingModel = class {
  ...
  constructor(config) {
    ...
    if (normalizedConfig.url) {
      ...
      this.providerModel = chunkCSHG5LWU_cjs.createOpenAICompatible({
      ...
      const registry = chunkD4M6E4OQ_cjs.GatewayRegistry.getInstance();
      ...
      if (normalizedConfig.providerId === "openai") {
        this.providerModel = chunkCSHG5LWU_cjs.createOpenAI({ apiKey }).textEmbeddingModel(
          normalizedConfig.modelId
        );
      } else if (normalizedConfig.providerId === "google") {
        this.providerModel = chunkCSHG5LWU_cjs.createGoogleGenerativeAI({ apiKey }).textEmbedding(
          normalizedConfig.modelId
        );
      } else {
...
//# sourceMappingURL=chunk-WU2P7XOU.cjs.map
//# sourceMappingURL=chunk-WU2P7XOU.cjs.map