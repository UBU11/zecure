...
var chunkV5YNS2QO_cjs = require('../chunk-V5YNS2QO.cjs');
...
var planCounter = 0;
var askUserTool = chunkD6HO5QAM_cjs.createTool({
  id: "ask_user",
...
var submitPlanTool = chunkD6HO5QAM_cjs.createTool({
  id: "submit_plan",
...
var taskWriteTool = chunkD6HO5QAM_cjs.createTool({
  id: "task_write",
...
var taskCheckTool = chunkD6HO5QAM_cjs.createTool({
  id: "task_check",
...
- The task is self-contained and can be delegated`,
    ...
    execute: async ({ agentType, task, modelId }, context) => {
      ...
      const subagent = new chunkYIJZBU54_cjs.Agent({
...
var Harness = class {
  ...
  constructor(config) {
    ...
    if (config.workspace instanceof chunk4P35AVPE_cjs.Workspace) {
  ...
  async init() {
    if (this.config.storage) {
      this.#internalMastra = new chunkV5YNS2QO_cjs.Mastra({ logger: false, storage: this.config.storage });
      await this.#internalMastra.getStorage().init();
    }
    if (this.config.workspace && !this.workspaceInitialized && !this.workspaceFn) {
      try {
        if (!this.workspace) {
          this.workspace = new chunk4P35AVPE_cjs.Workspace(this.config.workspace);
        }
  ...
  async getCurrentModelAuthStatus() {
    ...
    try {
      const { PROVIDER_REGISTRY } = await import('../provider-registry-4HLP2JRR.cjs');
...
\`\`\`` };
   ...
   */
  async processStream(response, requestContext) {
    let currentMessage = {
      id: this.generateId(),
      role: "assistant",
      content: [],
