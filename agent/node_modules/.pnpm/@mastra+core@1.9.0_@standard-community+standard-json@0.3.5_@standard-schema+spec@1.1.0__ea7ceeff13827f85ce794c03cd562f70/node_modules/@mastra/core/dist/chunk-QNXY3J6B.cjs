...
var CoreToolBuilder = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  constructor(input) {
    ...
    if (!chunkD6HO5QAM_cjs.isVercelTool(this.originalTool) && (input.autoResumeSuspendedTools || this.originalTool.id?.startsWith("agent-") || this.originalTool.id?.startsWith("workflow-"))) {
  ...
  getParameters = () => {
    if (chunkD6HO5QAM_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType, processedSchema) {
    ...
    const execFunction = async (args, execOptions) => {
      ...
      try {
        ...
        if (chunkD6HO5QAM_cjs.isVercelTool(tool)) {
          result = await chunk4UOWNFAL_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunk4UOWNFAL_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            ...chunk4UOWNFAL_cjs.createObservabilityContext({ currentSpan: toolSpan }),
          ...
          if (resumeData) {
            const resumeValidation = chunkD6HO5QAM_cjs.validateToolInput(resumeSchema, resumeData, options.name);
          ...
          result = await chunk4UOWNFAL_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
          const suspendSchema = this.getSuspendSchema();
          const suspendValidation = chunkD6HO5QAM_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkD6HO5QAM_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkD6HO5QAM_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      try {
        ...
        const { data, error: error2 } = chunkD6HO5QAM_cjs.validateToolInput(parameters, args, options.name);
        if (error2) {
  ...
  build() {
    ...
    let needsApprovalFn;
    if (chunkD6HO5QAM_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkD6HO5QAM_cjs.Tool) && !chunkD6HO5QAM_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-QNXY3J6B.cjs.map
//# sourceMappingURL=chunk-QNXY3J6B.cjs.map